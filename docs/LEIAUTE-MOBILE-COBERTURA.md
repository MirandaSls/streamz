...
| `DMChannelView` (contrato) | na captura, cada conversa mostra a **última mensagem** embaixo do nome; o nosso DTO não trazia esse texto. Não era leiaute: precisava de campo novo no contrato e no DTO, ou seja `apps/api` | **resolvido** — `DMChannelView.ultimaMensagem` (`{ id, authorId, content, createdAt, tipo }`), preenchido em `GET /dms` e desenhado por `linhaDaPrevia` no desktop e no celular |

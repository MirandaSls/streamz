 ...
 *
 * `PollVoters` (contrato) só tem `index` por opção, sem o texto — quem tem o
 * texto é a `Poll` que já está na store (`PollCard.tsx` semeia com `seed()`
 * assim que a mensagem renderiza, e este modal só abre a partir do botão
 * "Quem votou" de um card já na tela, então a enquete já está semeada). Por
 * isso o título de cada grupo usa `poll.options` para mostrar a resposta de
 * verdade em vez de "Opção 1", "Opção 2"… — não medido num print (não há
 * referência do painel "who voted" do Discord no material), mas é o dado
 * real que existe e o `PollVoters` sozinho não dá pra mostrar.
 */
export default function PollVotersModal({ messageId }: { messageId: string }) {
  const closeModal = useUI((s) => s.closeModal);
  const poll = usePolls((s) => s.polls[messageId]);
      ...
      {voters === null ? (
        ...
        <div className="flex flex-col gap-4">
          {voters.byOption.map((o) => {
            const texto = poll?.options.find((op) => op.index === o.index)?.text;
            return (
              <div key={o.index}>
                <h3 className="mb-1 text-xs font-bold uppercase tracking-[0.02em] text-text-subtle">
                  {texto ?? `Opção ${o.index + 1}`} — {o.users.length} {o.users.length === 1 ? "voto" : "votos"}
                </h3>
                {o.users.length === 0 ? (

...
import { displayNameOf, isUnread, type Category, type Channel } from "@newdisc/shared";
...
import { useVoiceParticipants } from "@/stores/voiceStates";
...
/**
 * Quem está num canal de voz, ao vivo (evento `voice.state` do gateway).
 * Enquanto ninguém entra, a lista simplesmente não aparece.
 */
function VoiceMembers({ channelId }: { channelId: string }) {
  const participantes = useVoiceParticipants(channelId);
  if (participantes.length === 0) return null;
  return (
    <ul className="mb-0.5 ml-6 mr-2 space-y-0.5">
      {participantes.map((v) => (
        <li key={v.user.id}>
          <button
            type="button"
            onClick={(e) => ui.openProfile(v.user, anchorOf(e.currentTarget))}
            className="flex h-7 w-full items-center gap-1.5 rounded-[4px] px-2 text-left text-sm text-txt-faint transition hover:bg-hov hover:text-txt-normal"
          >
            <Avatar user={v.user} size="sm" className={v.deafened || v.muted ? "opacity-50" : ""} />
            <span className="min-w-0 flex-1 truncate">{displayNameOf(v.user)}</span>
            {v.screen && <MonitorUp size={14} className="shrink-0 text-green" aria-label="Transmitindo tela" />}
            {v.video && <Video size={14} className="shrink-0 text-green" aria-label="Câmera ligada" />}
            {v.deafened ? (
              <HeadphoneOff size={14} className="shrink-0 text-red" aria-label="Surdo" />
            ) : (
              v.muted && <MicOff size={14} className="shrink-0 text-red" aria-label="Sem microfone" />
            )}
          </button>
        </li>
      ))}
  ...
  function renderChannel(channel: Channel, grupo: CategoryGroup, index: number) {
    ...
    return (
      <div key={channel.id}>
        ...
        {channel.type === "VOICE" && <VoiceMembers channelId={channel.id} />}
      </div>
    );
  }

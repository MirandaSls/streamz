      ...
      {aberto &&
        ...
        createPortal(
          <div
            ...
            className="pointer-events-none fixed z-[100] w-[220px] rounded-lg bg-rail p-3 text-center shadow-high anim-menu"
          >
            ...
            <span className="mt-2 flex items-center justify-center -space-x-1.5">
              {visiveis.map((u, i) =>
                u ? (
                  <Avatar key={u.id} user={u} size="sm" className="ring-2 ring-rail" />
                ) : (
                  <span
                    key={`x${i}`}
                    className="grid h-6 w-6 place-items-center rounded-full bg-panel text-[10px] text-txt-muted ring-2 ring-rail"
                  >
                    ?
                  </span>
                ),
              ...
              {sobra > 0 && (
                <span className="grid h-6 w-6 place-items-center rounded-full bg-panel text-[10px] font-semibold text-txt-muted ring-2 ring-rail">
                  +{sobra}
                </span>
              )}
            </span>

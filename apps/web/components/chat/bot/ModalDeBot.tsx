...
import { useMemo, useState, type KeyboardEvent as KeyboardEventDoReact } from "react";
import type {
  Channel,
  ComponenteDeModal,
  EmojiParcial,
  FilhoDeLabel,
  Label as LabelDeModal,
  RespostaDeCampoDeModal,
  RespostaDeComponenteDeModal,
  Role,
  TextInput as CampoDeTextoDoBot,
} from "@streamz/shared";
import {
  Button,
  Campo,
...
import { Hash, Lock, Megaphone, Shield, Upload, Volume2 } from "@/components/ui/icones";
import { Markdown } from "@/lib/markdown";
import Avatar from "@/components/ui/Avatar";
import Emoji from "@/components/ui/Emoji";
import { MensagemDeAjuda } from "@/components/settings/aplicativos/pecas";
 ...
 * arquivo, aqui desenhada mas inerte — ver `nao_verificado`/`faltando` do
 * cartão), `modal-na-visao-geral.png` (a forma ANTIGA, sem `Label`: rótulo em
 ...
 * (`select-opcoes.ts`, `select-canais.ts`, dos dois exportados) — só a
 * montagem da lista de opções é reescrita aqui (não é exportada de
 * `SelectDeBot.tsx` para reaproveitar de lá).
 *
...
function validarCampo(campo: FilhoDeLabel, valor: ValorDoCampo): string | null {
  switch (campo.type) {
    ...
    case 19:
      // sem upload implementado (ver o cabeçalho do arquivo): só bloqueia quando o bot exige um arquivo.
      return ehObrigatorio(campo) ? "Envio de arquivo ainda não é suportado aqui." : null;
    case 21:
...
function respostaDoCampo(campo: FilhoDeLabel, valor: ValorDoCampo): RespostaDeCampoDeModal {
  ...
  switch (campo.type) {
    ...
    case 19:
      return { type: 19, id, custom_id: campo.custom_id, values: [] };
    case 21:
...
function iconeDeCanal(type: Channel["type"]) {
  if (type === "VOICE") return <Volume2 size={16} aria-hidden="true" className="text-icon-muted" />;
  if (type === "ANNOUNCEMENT") return <Megaphone size={16} aria-hidden="true" className="text-icon-muted" />;
  return <Hash size={16} aria-hidden="true" className="text-icon-muted" />;
}

function opcaoDeUsuario(user: { id: string; username: string; displayName: string | null; avatarUrl: string | null }): OpcaoDeSelect<string> {
  return {
    valor: user.id,
    rotulo: user.displayName ?? user.username,
    prefixo: <Avatar user={{ id: user.id, username: user.username, avatarUrl: user.avatarUrl }} size="xs" />,
  };
}
...
function opcoesDeCargo(roles: readonly Role[]): OpcaoDeSelect<string>[] {
  return roles
    ...
    .map((r) => ({
      ...
      rotulo: r.name,
      prefixo: (
        <Shield size={16} aria-hidden="true" className={r.color ? "" : "text-icon-muted"} style={r.color ? { color: r.color } : undefined} />
      ),
    }));
}
...
function CampoDeSelect({
  ...
  const opcoes = useMemo<OpcaoDeSelect<string>[]>(() => {
    ...
    if (campo.type === 8) {
      return canaisFiltrados(canais, campo.channel_types, "").map((c) => ({
        valor: c.id,
        rotulo: c.name ?? "",
        prefixo: (
...
/** A área de soltar arquivo do `modal-file-upload.webp` — só o desenho; não envia nada (ver o cabeçalho do arquivo). */
function CampoDeArquivo() {
  return (
    <div>
      <div className="flex flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-border-subtle bg-background-base-lower px-4 py-6 text-center opacity-60">
        <Upload size={20} aria-hidden="true" className="text-icon-muted" />
        <p className="text-text-sm text-text-muted">Arraste arquivos aqui</p>
      </div>
      <p className="mt-1 text-text-xs text-text-muted">Envio de arquivo por aqui ainda não é suportado.</p>
    </div>
  );
}

...
export function BarraDeAlteracoes({ controle }: { controle: ControleDeAlteracoes }) {
  ...
  return (
    <div
      ref={barraRef}
      role="status"
      className="sticky bottom-5 z-20 mt-6 flex items-center gap-4 rounded-[8px] border border-border bg-overlay px-4 py-2.5 shadow-high"
    >
      <p className="min-w-0 flex-1 text-sm text-txt-primary">
        Cuidado — você tem alterações não salvas!
      </p>
      <button
        type="button"
        onClick={() => controle.acoes.current?.redefinir()}
        className="h-8 shrink-0 rounded-[3px] px-3 text-sm text-txt-normal underline-offset-2 transition hover:underline"
      >
        Redefinir
      </button>
      <button
        ...
        disabled={salvando}
        onClick={() => void salvar()}
        className="h-8 shrink-0 rounded-[3px] bg-accent px-4 text-sm font-medium text-accent-ink transition hover:bg-accent-hover disabled:opacity-50"
      >
